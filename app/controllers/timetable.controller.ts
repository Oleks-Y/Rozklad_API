...
export class TimetableController extends Controller {
  ...
  // user id in headers
  // Todo must be 400-like error if no headers
    ...
    try {
      ...
      await Promise.all(
        user?.subjects.map(async (subject) => {
          let lesson = await Lesson.find({ subject: subject })
            .populate({
              path: "subject",
              model: Subject,
            })
  ...
  private createTimetable(lessons_selected: ILesson[]) {
    ...
    for (let week = 1; week < /*Number of weeks++*/ 3; week++) {
      ...
      for (let day = 1; day < /*Number of days++*/ 7; day++) {
        let lessons_by_this_day = lessons_selected.filter(
          (lesson) => lesson.week === week && lesson.dayOfWeek === day
        );
